...
-- Canonical correlation schema (v2)
create table if not exists mapa_semantic.semantic_chunks_v2
(
  canonical_id_v2 String,
...
create table if not exists mapa_semantic.semantic_signals_v2
(
  canonical_id_v2 String,
  canonical_subject_id String,
  entity_kind LowCardinality(String),
...
create or replace view mapa_semantic.semantic_signals_summary_v2
as
select
  entity_kind,
  count() as events,
...
from mapa_semantic.semantic_signals_v2
group by entity_kind;

-- Compatibility view name consumed by middleware/frontend
create or replace view mapa_semantic.semantic_signals_summary_v1
as
select
  entity_kind,
  events,
  ...
  inflection_points,
  tacit_basis_signals
from mapa_semantic.semantic_signals_summary_v2;
